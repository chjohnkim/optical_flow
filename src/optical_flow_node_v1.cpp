...
void imageCallback(const sensor_msgs::Image::ConstPtr &image_msg) {
    ...
    {   cout << "Initializing..." << endl; 
        img1.copyTo(img0);
        cv::goodFeaturesToTrack(img0, p0, 50, 0.01, 10, Mat(), 3.0, 0.0, 0.04);
        cv::cornerSubPix(img0, p0, subPixWinSize, Size(-1,-1), termcrit);
    }
    ...
    //cout << height_time << "   " << last_height_time << "   " << (height_time - last_height_time)  << "   " << vz << endl;
    ...
    vx = (pixel_vx - (cx/height)*(velocity_gt[2]))/(-fx/height);
    vy = (pixel_vy - (cy/height)*(velocity_gt[2]))/(-fy/height);
